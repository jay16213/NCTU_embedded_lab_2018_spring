    ...
    def run(self):
        ...
        while 1:
            ...
            if (keys[K_RIGHT]):
                #Rotation
                self.rotate("RIGHT")
            #delta_t = pygame.time.get_ticks() - prev_time
            #prev_time = pygame.time.get_ticks()
            self.angleX, self.angleZ, self.angleY = sensor.getTiltDegree(self.angleX, self.angleZ, self.angleY)
            #self.angleZ, self.angleX, self.angleY = sensor.getTiltDegree(self.angleZ, self.angleX, self.angleY, delta_t)
            self.rotate()
            self.screen.fill(bgColor)
